...
class TestLMSAccessibility:
    def test_homepage(self, http_session: requests.Session, lms_url: str) -> None:
        resp = http_session.get(f"{lms_url}/", timeout=HTTP_TIMEOUT, allow_redirects=True)
    ...
    def test_login_page(self, http_session: requests.Session, lms_url: str) -> None:
        resp = http_session.get(f"{lms_url}/login", timeout=HTTP_TIMEOUT, allow_redirects=True)
    ...
    def test_register_page(self, http_session: requests.Session, lms_url: str) -> None:
        resp = http_session.get(f"{lms_url}/register", timeout=HTTP_TIMEOUT, allow_redirects=True)
        assert resp.status_code == 200, f"LMS /register returned HTTP {resp.status_code}"
    ...
    def test_heartbeat(self, http_session: requests.Session, lms_url: str) -> None:
        ...
        assert isinstance(resp.json(), dict), "Heartbeat response should be a JSON object"


class TestCMSAccessibility:
    def test_homepage(self, http_session: requests.Session, cms_url: str) -> None:
        resp = http_session.get(f"{cms_url}/", timeout=HTTP_TIMEOUT, allow_redirects=True)
    ...
    def test_signin_page(self, http_session: requests.Session, cms_url: str) -> None:
        resp = http_session.get(f"{cms_url}/signin", timeout=HTTP_TIMEOUT, allow_redirects=True)
