...
def get_compose_mounts(user_mounts: list[str], name: str) -> list[str]:
    """
    TODO document me.
    TODO optimize me: otherwise mounts are parsed many, many times.
    """
    mounts: list[str] = []
    for user_mount in user_mounts:
        for service, host_path, container_path in bindmount.parse_mount(user_mount):
            if service == name:
                mounts.append(f"{host_path}:{container_path}")
    return mounts

hooks.Filters.ENV_TEMPLATE_VARIABLES.add_item(("get_compose_mounts", get_compose_mounts))
